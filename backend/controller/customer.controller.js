    ...
    try {
        ...
        let query = `
            SELECT c.*,
                   ...
                   COALESCE(
                       (
                           SELECT JSON_AGG(JSON_BUILD_OBJECT('id', e.id, 'full_name', e.full_name))
                           FROM employees e
                           WHERE e.id IN (
...
export async function createCustomer(req, res) {
    ...
    try {
        const { name, branches, contactPersons, slaType, slaResponseTime, slaResolutionTime, contractStartDate, contractEndDate, deadline, industry } = req.body;
        ...
        const query = `
            INSERT INTO customers (name, branches, contact_persons, sla_type, sla_response_time, sla_resolution_time, contract_start_date, contract_end_date, deadline, industry, created_by)
...
export async function updateCustomer(req, res) {
    ...
    try {
        ...
        const query = `
            UPDATE customers
            SET name = $1, branches = $2, contact_persons = $3, sla_type = $4, sla_response_time = $5, sla_resolution_time = $6, contract_start_date = $7, contract_end_date = $8, deadline = $9, industry = $10, updated_at = CURRENT_TIMESTAMP
